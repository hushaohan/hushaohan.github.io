...
function render_venue(venue)
    ...
    if venue.volume ~= nil then
        table.insert(venue_items, string.format("Volume %d", venue.volume))
    end
    if venue.number ~= nil then
        table.insert(venue_items, string.format("Number %d", venue.number))
    end
    if venue.page ~= nil then
        table.insert(venue_items, string.format("Page %d", venue.page))
    end
...
function render_publication(publication)
    ...
    local publication_str = string.format("\\textbf{\\textsc{\\scriptsize %s %d}} & %s, \\href{%s}{\\emph{%s}}, %s",
        venue.acronym,
        venue.year,
...
tex.sprint(table.concat(utils.map(utils.load_db("../db/publications.json"), render_publication), "\\\\\\\\"))
